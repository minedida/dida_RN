...
布局holder ref=this.scrollPad
   1.holder包含touchIn、touchEnd、startDrag、afterDrag方法
     touchIn -> headerState='touched'
     touchEnd -> {
       this.scrollPad._component.scrollTo({x: 0, y: 0})
     }
     startDrag -> {
       headerState='touched'
       this.knobTracker.reset()
     ...
     afterDrag -> {
        if(snapY > total / 2) {
          this.scrollPad._component.scrollTo({x: 0, y: maxY})
        } else {
          this.state.calendarScrollable = true
        }
     }
    2.该holder是scrollview，同时滚动事件和state.scrollY进行绑定
    3.不理解为什么holder是scrollview


核心日历视图 ref=this.calendar
...
HEADER_HEIGHT = 104
KNOB_HEIGHT = 24
this.viewHeight 749 -> 711
this.viewWidth  414
agendaHeight = this.viewHeight - 104 = 607
*/
